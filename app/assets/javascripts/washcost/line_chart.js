...
window.WashCostLineChart = (function() {
	...
	WashCostLineChart.prototype = {
		...
		init: function(options) {
			...
			this.padding = Helper.extend({
				top: 0,
				right: 0,
				bottom: 0,
				left: 0
			}, options.padding);
			this.axisTypes = Helper.extend({
				x: null,
				y: null,
				z: null
			}, options.axisTypes);
			this.dataAccessors = Helper.extend({
				x: null,
				y: null,
				z: null
			}, options.dataAccessors);
			this.dataPosAccessors = options.dataPosAccessors || {
				x: function(d) {
					return self.xScale(self.dataAccessors.x(d));
				},
				y: function(d) {
					return self.yScale(self.dataAccessors.y(d));
				},
				z: function(d) {
					return self.zScale(self.dataAccessors.z(d));
				}
			};
			this.seriesNameAccessor = options.seriesNameAccessor || function(d) {
				return d.seriesName
			};
			...
			this.toolTipStyle = Helper.extend({
				...
				shadowRadius: 5,
				shadowColor: '#000',
				textFamily: 'Helvetic Neue, Helvetica, Arial',
				textColour: 'black',
			...
			this.brushOptions = Helper.extend({
				parent: null,
				dimensions: {
					width: 100,
					height: 25
				...
				focusBackgroundColour: 'transparent',
				shadeOpacity: 0.0,
				handles: {
					width: 20,
					image: null
			...
			if (this.axisTypes.x) {
				...
				if (this.axisTypes.x === 'time') {
					this.xAxis.ticks(this.timeAxisTickInterval);
				}
				if (!this.dataAccessors.x) {
					this.dataAccessors.x = this.defaultAccessorForAxisType(this.axisTypes.x);
				}
			}
			if (this.axisTypes.y) {
				...
				if (this.axisTypes.y === 'time') {
					this.yAxis.ticks(this.timeAxisTickInterval);
				}
				if (!this.dataAccessors.y) {
					this.dataAccessors.y = this.defaultAccessorForAxisType(this.axisTypes.y);
				}
			}
			if (this.axisTypes.z) {
				...
				if (this.axisTypes.z === 'time') {
					this.zAxis.ticks(this.timeAxisTickInterval);
				}
				if (!this.dataAccessors.z) {
					this.dataAccessors.z = this.defaultAccessorForAxisType(this.axisTypes.z);
				}
			}
			...
			if (this.hasBrush) {
				...
				this.brush = {
					xScale: brushXScale,
					yScale: brushYScale,
					constructor: d3.svg.brush().x(brushXScale).on('brush', function() {
						self.brushed.apply(self, arguments)
					})
				};
			...
			if (this.shouldZoom) {
				...
				this.zoom = d3.behavior.zoom()
					.on('zoom', function() {
						self.redraw.apply(self, arguments)
					})
					.scaleExtent([1, 20]);
...
*/
		...
		render: function(data) {
			...
			xScale.domain(
				[
					d3.min(this.data, function(s) {
						return d3.min(s.data, dataAccessors.x);
					}),
					d3.max(this.data, function(s) {
						return d3.max(s.data, dataAccessors.x);
					})
				])
				.range([0, width])
				.nice();
			...
			yScale.domain(
				[
					d3.min(this.data, function(s) {
						return d3.min(s.data, dataAccessors.y);
					}),
					d3.max(this.data, function(s) {
						return d3.max(s.data, dataAccessors.y);
					})
				])
				.range([height, 0])
				.nice();
			...
			this.middleLineConstructor = d3.svg.line()
				...
				.y(function(d) {
					return yScale((self.yScale.domain()[1] - self.yScale.domain()[0]) / 2);
				})
				.interpolate(this.interpolation);
			...
			this.lines = this.series.selectAll('path.dataLine')
				...
				.attr("d", function(d) {
					return self.middleLineConstructor(d.data);
				})
				...
				.style('stroke', function(d, i) {
					var colour = d.colour || colourPalette(i);
					return colour;
				})
				.style('fill', 'none')
				.style('opacity', 0);
			...
			this.canvas.selectAll('path.dataLine').transition().duration(800)
				...
				.attr('d', function(d) {
					return self.lineConstructor(d.data);
				});
		...
		renderAxis: function(axis) {
			...
			if (this.xAxis && axis === 'x') {
				...
				this.xAxisElement = this.canvas.append("g")
					...
					.attr("transform", function() {
						var axisPos = self.yScale(0);
						if (axisPos > self.height || axisPos < 0 || self.axisTypes.y === 'ordinal') axisPos = self.height;
						return "translate(0," + axisPos + ")";
					})
			...
			if (this.yAxis && axis === 'y') {
				...
				this.yAxisElement = this.canvas.append("g")
					.attr("class", "y axis")
					.attr("transform", function() {
						var axisPos = self.xScale(0);
						if (axisPos > self.width || axisPos < 0 || self.axisTypes.x === 'ordinal' || self.axisTypes.x === 'time') axisPos = 0;
						return "translate(" + axisPos + ", 0)";
					})
			...
			if (!this.ie8Detected) {
				if (this.xAxis) {
					this.canvas.select('g.x.axis').transition().duration(this.primaryAnimationDuration)
						.attr("transform", function() {
							var axisPos = self.yScale(0);
							if (axisPos > self.height || axisPos < 0) axisPos = self.height;
							return "translate(0," + axisPos + ")";
						})
						.call(this.xAxis);
				}
		...
		repositionAxes: function() {
			...
			// axes themselves
			this.canvas.select('g.x.axis').attr("transform", function() {
				var axisPos = self.yScale(0);
				if (axisPos > self.height || axisPos < 0 || self.axisTypes.y === 'ordinal') axisPos = self.height;
				return "translate(0," + axisPos + ")";
			});
			this.canvas.select('g.y.axis').attr("transform", function() {
				var axisPos = self.xScale(0);
				if (axisPos > self.width || axisPos < 0 || self.axisTypes.x === 'ordinal' || self.axisTypes.x === 'time') axisPos = 0;
				return "translate(" + axisPos + ", 0)";
			});
		...
		*/
		...
		redraw: function() {
			...
			// lines
			this.canvas.selectAll('.series path.dataLine').attr("d", function(d) {
				return lineConstructor(d.data);
			});
		...
		nearestYDataPoints: function(yPos, allDataPoints, number) {
			...
			for (i = 0; i < allDataPoints.length; i++) {
				...
				pointsByDistance.push({
					distance: distance,
					dataPoint: allDataPoints[i]
				});
			}
		...
		linearRangeWithPadding: function(inputRange, paddingPercent) {
			...
			if (this.shouldLockAxesToPositive)
				if (inputRange[0] >= 0 && newMin < 0) newMin = 0;
		...
		*/
		...
		findLineOfBestFit: function(xValues, yValues, xInput) {
			...
			xValues.forEach(function(dp) { // sum total of all the x values
				sumX += dp;
			});

			yValues.forEach(function(dp) { // sum total of all the y values
				sumY += dp;
			});

			xValues.forEach(function(dp) { // sum of all the squared x values
				sumX2 += dp * dp;
			});

			xValues.forEach(function(dp, i) { // sum of all the products of x and y
				sumXY += dp * yValues[i];
			});
