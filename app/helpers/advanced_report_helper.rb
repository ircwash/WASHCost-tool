module AdvancedReportHelper
  ...
  def report_currency_value_for( value, precision = 2 )
    ...
    value != nil ? "#{number_with_precision( number_to_currency(value.to_f, :locale => I18n.locale, :unit => currency.symbol), :precision => precision )}" : t( 'report.no_data' )
  end
  ...
  def report_delta_currency_value_for( value, precision = 2 )
    ...
    if (value != nil && value == 0.00)
      ...
      value != nil ? "#{number_with_precision( number_to_currency(value.to_f.abs, :locale => I18n.locale, :unit => currency.symbol), :precision => precision )} #{value.to_f > 0 ?  t( 'report.surplus' ) : t( 'report.shortfall' )}" : t( 'report.no_data' )
    end
  end
